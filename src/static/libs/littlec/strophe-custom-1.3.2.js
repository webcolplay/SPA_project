...
Strophe = {
    ...
    NS: {
        ...
        FILEMSGEX:"jabber:x:fileMessageExtention"
    },
     ...
     *  Status.NETWORKERR - An networkerror has occurred
     */
    Status: {
        ERROR: 0,
        CONNECTING: 1,
...
Strophe.Builder.prototype = {
     ...
     *    The Strophe.Builder object.
     */
    c: function (name, attr, text)
    {
        var child = Strophe.xmlElement(name, attr, text);
    ...
    nc: function (names, attrs, texts)
    {
        var len = names.length;
        for(var i=0;i<len;i++){
            var child = Strophe.xmlElement(names[i], attrs[i], texts[i]);
            this.node.appendChild(child);
        }
        return this;
    },
...
Strophe.Handler.prototype = {
    ...
    isMatch: function (elem)
    {
        ...
        // var flags = !this.name || Strophe.isTagEqual(elem, this.name);
        // console.log(flags);
        //  flags = !this.type || elem.getAttribute("type") == this.type;
        // console.log(flags);
        //  flags = !this.id || elem.getAttribute("id") == this.id;
        //   console.log(flags);
        //  flags = !this.from || from == this.from;
        // console.log(flags);
        if (nsMatch &&
...
//        }
    ...
    connectforRegister: function (callback,domain,wait, hold)
    {
        this.servtype = "xmpp";
        this.disconnecting = false;
        this.connected = false;
        ...
        this._proto._connect(wait, hold);
    },
    /** Function: attach
    ...
    */
    ...
    _doDisconnect: function (reason)
    {
        ...
        if(reason == "network"){
            this._changeConnectStatus(Strophe.Status.NETWORKERR, "网络中断或服务器异常导致断开连接，请重新登录！");
        }else{
    ...
    _dataRecv: function (req, raw)
    {
        ...
        //from pubulic do remove
        if(from&&(from.indexOf('@pubacct')!=-1)){
            return;
        }
        ...
        Strophe.forEachChild(elem, null, function (child) {
            ...
            for (i = 0; i < newList.length; i++) {
                ...
                try {
                    ...
                    }else if(hand.isMatch(child) && that.isRegister){
                        that.handlers.push(hand);
                        hand.run(child);
                    }
                    else {
     ...
     *    false to remove the handler.
     */
    _onDisconnectTimeout: function (reason)
    {
...
Strophe.Bosh.prototype = {
    ...
    _connect: function (wait, hold, route)
    {
        ...
        var body = this._buildBody().attrs({
            ...
            wait: this.wait,
            hold: this.hold,
            inactivity : this._conn.inactivity,
        ...
        if (req.xhr.readyState == 4) {
            ...
            if (reqStatus == 200) {
                ...
                Strophe.error("request id " +
                              ...
                              " happened");
                if(req.sends >1 && req.sends<=this.maxRetries){
                    this._conn._changeConnectStatus(Strophe.Status.NETWORKERR, "网络错误，请求失败");
                }else if(req.sends > this.maxRetries){
                    this._conn._onDisconnectTimeout('network');
                    networkStatus = false;
                    return;
                }
                if (reqStatus === 0 ||
